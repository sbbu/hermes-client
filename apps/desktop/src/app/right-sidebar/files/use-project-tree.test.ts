...
describe('useProjectTree', () => {
  ...
  it('keeps an unreadable root visible while a same-root refresh probes', async () => {
    readDir.mockResolvedValueOnce({ entries: [], error: 'ENOENT' })
    const { result } = renderHook(() => useProjectTree('/gone'))
    await waitFor(() => expect(result.current.rootError).toBe('ENOENT'))

    let release: ((value: HermesReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(() => new Promise(resolve => (release = resolve)))
    act(() => void result.current.refreshRoot())

    expect(result.current.rootLoading).toBe(true)
    expect(result.current.rootError).toBe('ENOENT')
    await act(async () => release?.({ entries: [], error: 'ENOENT' }))
  })

  it('clears same-path rows when the backend changes', async () => {
    ...
    let release: ((value: HermesReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(() => new Promise(resolve => (release = resolve)))
    act(() => $connection.set({ baseUrl: 'local-b', mode: 'local' } as never))

    expect(result.current.data).toEqual([])
    await act(async () => release?.(ok([{ name: 'from-b', path: '/shared/from-b', isDirectory: false }])))
