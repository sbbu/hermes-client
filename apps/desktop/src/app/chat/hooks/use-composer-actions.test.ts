...
describe('partitionDroppedFiles', () => {
  it('routes File-bearing OS drops to osDrops and path-only in-app drags to inAppRefs', () => {
    const finderPdf = osDrop('/Users/example/Downloads/DEVIS_signed.pdf')
  ...
  it('routes an OS folder drop (path-only, isDirectory) to inAppRefs, not the upload pipeline', () => {
    ...
    const folder = inAppRef('/Users/example/projects/hermes', { isDirectory: true })
...
describe('extractDroppedFiles', () => {
  ...
  it('emits a dropped directory as a path-only entry with isDirectory (no File to upload)', () => {
    const transfer = stubTransfer([{ path: '/Users/example/projects/hermes', isDirectory: true }]) as DataTransfer & {
    ...
    expect(result[0]?.path).toBe('/Users/example/projects/hermes')
  ...
  it('still emits a dropped file with its native File handle for the upload pipeline', () => {
    const transfer = stubTransfer([
      { path: '/Users/example/Downloads/report.pdf', isDirectory: false }
    ...
    expect(result[0]?.path).toBe('/Users/example/Downloads/report.pdf')
...
describe('attachmentPreviewDataUrl', () => {
  ...
  it('reads a local path via the local bridge even in remote mode (paperclip/paste/OS drop)', async () => {
    ...
    await expect(attachmentPreviewDataUrl('/Users/example/Pictures/pic.png')).resolves.toBe(LOCAL_PREVIEW)

    expect(readFileDataUrl).toHaveBeenCalledWith('/Users/example/Pictures/pic.png')
