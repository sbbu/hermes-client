...
function Harness({
  ...
  const actions = usePromptActions({
    ...
    getRouteToken: getRouteToken ?? (() => 'test-route'),
    handleSkinCommand: () => '',
    refreshSessions,
    requestGateway,
...
describe('usePromptActions sleep/wake session recovery', () => {
  ...
  it('resumes the selected stored session instead of minting a new one when activeSessionId is null', async () => {
    ...
    expect(calls[0]?.params).toEqual({ session_id: STORED_SESSION_ID, source: 'desktop', omit_messages: true })
    expect(calls[1]?.params).toMatchObject({ session_id: RECOVERED_SESSION_ID })
  })

  it('aborts when the selected route changes while session.resume is pending', async () => {
    ...
    await expect(handle!.submitText('stay in the original chat')).resolves.toBe(false)
