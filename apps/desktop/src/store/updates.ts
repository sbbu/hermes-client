...
import { checkHermesUpdate } from '@/hermes'
...
// v5: supports the raised WebSocket frame size for large file.attach payloads.
 ...
 * cryptically downstream, surface a warning whose action lands on the fork-safe
 * manual update instructions instead of running the stock backend updater.
 *
...
const FORK_SAFE_BACKEND_UPDATE_MESSAGE =
  'remote Hermes updates are managed by the fork rebase/update workflow, not by the stock desktop updater.'

const FORK_SAFE_BACKEND_UPDATE_COMMAND = 'ask Hermes to run the Nightly Hermes Custom Update job'
...
export async function applyUpdates(opts: DesktopUpdateApplyOptions = {}): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    // `hermes-client update` themselves. Land on a dedicated manual state so the
    ...
    if (result?.manual) {
      $updateApply.set({
        ...
        applying: false,
        stage: 'manual',
        message: result.command ?? 'hermes-client update',
        command: result.command ?? 'hermes-client update'
      })
...
export async function applyBackendUpdate(): Promise<DesktopUpdateApplyResult> {
  dismissNotification(UPDATE_TOAST_ID)
  const message = FORK_SAFE_BACKEND_UPDATE_MESSAGE
  const command = FORK_SAFE_BACKEND_UPDATE_COMMAND
