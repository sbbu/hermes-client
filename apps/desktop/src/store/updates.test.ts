...
describe('checkBackendUpdates', () => {
  ...
  it('honours can_apply=false (docker/nix): not supported, carries message', async () => {
    ...
    expect(result?.supported).toBe(false)
    expect(result?.message).toMatch(/fork rebase\/update workflow/)
  })
...
describe('applyUpdates terminal state', () => {
  ...
  it('keeps the manual command state for CLI installs with no staged updater', async () => {
    applyMock.mockResolvedValue({ ok: true, manual: true, command: 'hermes-client update' })

    await applyUpdates()

    expect($updateApply.get().stage).toBe('manual')
    expect($updateApply.get().command).toBe('hermes-client update')
    expect($updateOverlayOpen.get()).toBe(true)
    expect(notifySpy).not.toHaveBeenCalled()
  })
...
describe('applyBackendUpdate fork-safe guard', () => {
  beforeEach(() => {
    storage.clear()
    checkHermesUpdateSpy.mockReset()
  ...
  it('never launches the stock remote backend updater from Hermes Client Desktop', async () => {
    const result = await applyBackendUpdate()

    expect(result).toMatchObject({ ok: false, manual: true, error: 'manual' })
    ...
    expect($backendUpdateApply.get()).toMatchObject({
      applying: false,
      stage: 'manual',
      command: 'ask Hermes to run the Nightly Hermes Custom Update job'
    })
  })
})
