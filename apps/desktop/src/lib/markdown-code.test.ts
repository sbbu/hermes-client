...
describe('isLikelyProseCodeBlock', () => {
  ...
  it('keeps an SSH config block fenced (regression: rendered as flat prose)', () => {
    const ssh = ['Host 192.0.2.1', '    HostName 192.0.2.1', '    User example', '    Port 22'].join('\n')
  ...
  it('keeps a flat key-value config fenced', () => {
    expect(isLikelyProseCodeBlock('', ['Host example', 'User example', 'Port 22'].join('\n'))).toBe(false)
  })
...
describe('isLikelyStructuredText', () => {
  ...
  it('flags flat key-value / settings listings', () => {
    expect(isLikelyStructuredText(['Host example', 'User example', 'Port 22'].join('\n'))).toBe(true)
