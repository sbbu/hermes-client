...
const CLIENT_UPDATE_COMMAND = 'hermes-client update'
const CLIENT_APP_ID = 'com.sbbu.hermesclient'

function hermesClientConfigDir() {
  return path.resolve(process.env.HERMES_CLIENT_HOME || path.join(app.getPath('home'), '.config', 'hermes-client'))
}

function hermesClientConfigPath() {
  return path.join(hermesClientConfigDir(), 'config.json')
}

function readHermesClientConfig() {
  try {
    const parsed = JSON.parse(fs.readFileSync(hermesClientConfigPath(), 'utf8'))
...
function defaultDesktopConnectionConfig() {
  const remoteUrl = String(process.env.HERMES_CLIENT_REMOTE_URL || readHermesClientConfig().base_url || '').trim()
  return {
    mode: 'remote',
    remote: remoteUrl ? { url: remoteUrl, authMode: 'token' } : { authMode: 'token' },
    profiles: {}
  }
}
...
function resolveHermesHome() {
  if (process.env.HERMES_CLIENT_DESKTOP_HERMES_HOME) {
    return normalizeHermesHomeRoot(process.env.HERMES_CLIENT_DESKTOP_HERMES_HOME)
  }
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'hermes-client-home')
  const base = process.env.HERMES_CLIENT_STATE_HOME
    ? path.resolve(process.env.HERMES_CLIENT_STATE_HOME)
    : IS_WINDOWS && process.env.LOCALAPPDATA
      ? path.join(process.env.LOCALAPPDATA, 'hermes-client')
      : path.join(app.getPath('home'), '.local', 'state', 'hermes-client')
  // Deliberately do not honor HERMES_HOME here. Hermes Client Desktop is a thin
  // remote client; using ~/.hermes would let the official desktop bootstrap or
  // updater mutate the full Hermes Agent checkout/fork on this machine.
  return path.join(base, 'desktop-hermes-home')
}
...
if (IS_WINDOWS) {
  app.setAppUserModelId(CLIENT_APP_ID)
}
...
app.setAboutPanelOptions({
  ...
  copyright: 'Copyright © 2026 Hermes Client contributors'
})
...
function isHermesClientSourceRoot(root) {
  return (
    directoryExists(root) &&
    fileExists(path.join(root, 'pyproject.toml')) &&
    fileExists(path.join(root, 'src', 'hermes_client', 'cli.py'))
  )
}
...
// Hermes Client updates target the thin-client repository, never the full
// Hermes Agent checkout. This keeps the desktop updater away from a local fork
// and avoids the official first-launch reinstall loop entirely.
function resolveUpdateRoot() {
  const candidates = [
    process.env.HERMES_CLIENT_UPDATE_ROOT && path.resolve(process.env.HERMES_CLIENT_UPDATE_ROOT),
    !IS_PACKAGED && isHermesClientSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || SOURCE_REPO_ROOT
}
...
async function checkUpdates() {
  ...
  if (!directoryExists(gitDir)) {
    return {
      ...
      message: `${updateRoot} isn't a git checkout — Hermes Client Desktop source updates only run against a hermes-client git checkout.`,
...
async function applyUpdates() {
  ...
  try {
    ...
    return {
      ...
      command,
      hermesRoot: updateRoot,
      message: 'Hermes Client updates use the hermes-client package/fork, not the stock Hermes Agent updater.'
    }
  } finally {
...
function readDesktopConnectionConfig() {
  ...
  let config = defaultDesktopConnectionConfig()
...
function extractDashboardSessionToken(html) {
  const match = String(html || '').match(/window\.__HERMES_SESSION_TOKEN__=["']([^"']+)["']/)
  return match ? match[1] : null
}

async function discoverDashboardSessionToken(baseUrl) {
  const html = await fetchText(`${baseUrl}/`, { timeoutMs: 8_000 })
  return extractDashboardSessionToken(html)
}
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (!staticToken) {
    staticToken = await discoverDashboardSessionToken(baseUrl)
  }
  if (!staticToken) {
    throw new Error(
      'Remote Hermes gateway is selected, but no session token is saved and none could be discovered from the dashboard. ' +
        'Open Settings → Gateway and save a token, or sign in if the gateway uses OAuth.'
    )
  }
  ...
  return {
    ...
    authMode: 'token',
    token: staticToken,
    wsUrl: buildGatewayWsUrl(baseUrl, staticToken)
  }
}
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    if (authMode !== 'oauth') {
      token = decryptDesktopSecret(block.token) || (await discoverDashboardSessionToken(baseUrl))
    }
  } else {
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (CLIENT_ONLY) {
    throw new Error(`Hermes Client Desktop is remote-only; profile "${profile}" has no remote backend configured.`)
  }
...
function createWindow() {
  ...
  // Start the backend in parallel with renderer load. getConnection() joins
  // the shared in-flight promise; missed progress is recovered by the renderer pull.
...
function terminalShellEnv() {
  ...
  env.TERM_PROGRAM = 'Hermes Client'
...
// src/hermes_client/__init__.py + pyproject.toml) so the desktop About panel
// shows the Hermes Client version. Falls back to app.getVersion() when the
// source tree can't be read (e.g. a packaged build without the repo).
function resolveHermesVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'src', 'hermes_client', '__init__.py')
