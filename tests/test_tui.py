...
def test_run_tui_overrides_inherited_local_agent_env(tmp_path, monkeypatch):
    ...
    def fake_call(argv, env):
        captured["argv"] = argv
        captured["env"] = env
        captured["active_session_path"] = Path(env["HERMES_TUI_ACTIVE_SESSION_FILE"])
        assert captured["active_session_path"].is_file()
        return 7
    ...
    assert not captured["active_session_path"].exists()


def test_run_tui_removes_active_session_file_when_node_launch_fails(tmp_path, monkeypatch):
    app_dir = tmp_path / "client-config"
    monkeypatch.setattr(tui, "APP_DIR", app_dir)
    monkeypatch.setattr(tui, "ensure_app_dir", lambda: app_dir.mkdir(parents=True, exist_ok=True))
    monkeypatch.setattr(tui, "resolve_node", lambda: "/mock/bin/node")
    ...
    class FakeDashboardClient:
        ...
        def websocket_url(self):
            return "ws://remote.example/api/ws?token=x"

    monkeypatch.setattr(tui, "DashboardClient", FakeDashboardClient)
    active_session_path = None

    def fail_call(argv, env):
        nonlocal active_session_path
        active_session_path = Path(env["HERMES_TUI_ACTIVE_SESSION_FILE"])
        assert active_session_path.is_file()
        raise OSError("node launch failed")

    monkeypatch.setattr(tui.subprocess, "call", fail_call)

    with pytest.raises(OSError, match="node launch failed"):
        tui.run_tui("http://remote.example")

    assert active_session_path is not None
